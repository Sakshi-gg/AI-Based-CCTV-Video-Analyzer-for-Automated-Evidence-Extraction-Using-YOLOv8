...
class AnalyzerWindow(QMainWindow):
    ...
    def apply_dark_style(self):
        ...
        QSpinBox, QLineEdit, QComboBox {{
            ...
            padding: 5px; border-radius: 3px; color: {COLOR_TEXT};
        }}
        QSlider::groove:horizontal {{ 
            border: 1px solid {COLOR_BORDER}; height: 8px; 
            background: {COLOR_SECONDARY}; margin: 2px 0; border-radius: 4px; 
        }}
        QSlider::handle:horizontal {{ 
            background: {COLOR_ACCENT}; border: 1px solid {COLOR_ACCENT}; 
            width: 18px; margin: -5px 0; border-radius: 9px; 
        }}
        QPushButton {{ 
            background-color: #555555; color: white; border-radius: 5px; 
            padding: 8px; border: 1px solid white; font-weight: bold;
        }}
        QPushButton:hover {{ background-color: #666666; border: 1px solid #666666; }}
        QGroupBox {{ 
            border: 1px solid white; margin-top: 2ex; 
            border-radius: 5px; padding-top: 10px; font-weight: bold; 
        }}
        QGroupBox::title {{ 
            subcontrol-origin: margin; subcontrol-position: top center; 
            padding: 0 3px; background-color: {COLOR_SECONDARY}; color: white; 
        }}
        QScrollArea {{ border: none; background-color: {COLOR_SECONDARY}; }}
        QCheckBox {{ padding: 5px; margin: 2px 0; }}
        QCheckBox::indicator {{
            width: 15px; height: 15px;
            border: 2px solid {COLOR_BORDER}; border-radius: 3px; background: #1E1E1E;  
        }}
    ...
    def setup_sidebar(self):
        ...
        self.upload_btn.setStyleSheet(f"QPushButton {{ background-color: {self.COLOR_UPLOAD_FILE}; color: white; border: none; }}")
        ...
        self.reset_btn.setStyleSheet(f"QPushButton {{ background-color: {self.COLOR_RESET_FILTERS}; color: black; border: none; }}")
        self.reset_btn.clicked.connect(self.reset_filters)
        upload_layout.addWidget(self.reset_btn)
        
        self.clear_evidence_btn = QPushButton("🗑️ Clear Evidence")
        self.clear_evidence_btn.setStyleSheet(f"QPushButton {{ background-color: {self.COLOR_CLEAR_EVIDENCE}; color: white; border: none; }}")
        ...
        self.generate_report_btn.setStyleSheet(f"QPushButton {{ background-color: {self.COLOR_REPORT_SAVE}; color: white; border: none; }}")
    ...
    def setup_content_area(self):
        ...
        self.anomaly_score_metric  = create_metric_label("⚠ Anomaly: 0.00")
        ...
        metrics_layout.addWidget(self.anomaly_score_metric)
    ...
    def select_video_file(self):
        ...
        if file_name:
            ...
            self.file_path_label.setText(f"File: {file_name.split('/')[-1]}")
            self.analyze_btn.setEnabled(True)
            self.extract_and_display_metadata(file_name)
            
            if self.total_frames > 0 and self.video_fps > 0:
                self.end_time_input.setText(seconds_to_hms(self.total_frames / self.video_fps))
            else:
    ...
    def start_analysis_from_ui(self):
        ...
        end_sec      = float('inf') if end_time_str == "99:99:99" else hms_to_seconds(end_time_str)
        ...
        self.start_analysis(self.current_video_path, start_sec, end_sec, self.color_select.currentText())
        self.analyze_btn.setText("◼️ STOP")
        self.analyze_btn.setStyleSheet(f"QPushButton {{ background-color: {self.COLOR_STOP_ANALYSIS}; color: white; border: none; }}")
    ...
    def start_analysis(self, video_path, start_sec, end_sec, color_filter):
        ...
        self.current_case_id = create_case(f"Case - {video_name} - {time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    @Slot(np.ndarray, int, float, float, int, float, bool)
    def update_frame(self, frame, detection_count, current_frame_pos, video_fps, frame_counter, anomaly_score, is_anomalous):
        ...
        color = "#FF4444" if is_anomalous else "#4CAF50"
        self.anomaly_score_metric.setText(f"⚠ Anomaly: {anomaly_score:.2f}")
        self.anomaly_score_metric.setStyleSheet(f"""
            QLabel {{
                background-color: #2C2C2C;
        ...
        if detection_count > 0:
            ...
            self.add_evidence_to_gallery(
                ...
                video_fps,
                detection_count,
                self.get_current_filter_settings(),
                anomaly_score
            )

    def add_evidence_to_gallery(self, frame_rgb, current_frame_pos, video_fps, detection_count, filter_settings, anomaly_score=0.0):
        current_time_seconds = (current_frame_pos - 1) / video_fps if current_frame_pos > 0 else 0.0
        ...
        caption_label = QLabel(f"Time: {timestamp_hms} | Det: {detection_count} | Anomaly: {anomaly_score:.2f}")
    ...
    def analysis_finished(self, total_time):
        ...
        self.processing_time_label.setText(f"Total Processing Time: {seconds_to_min_sec_string(total_time)}")
